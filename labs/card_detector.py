...
class UNOCardDetector:
    ...
    def _nms(self, detections: List[CardDetection], iou_threshold: float = 0.3) -> List[CardDetection]:
        """
    ...
    def _lowe_ratio_test(
        ...
        return good_matches, np.array(confidence_scores) if confidence_scores else np.array([])
    ...
    def _estimate_homography(
        ...
        src_pts = np.float32([keypoints_test[m.queryIdx].pt for m in matches]).reshape(-1, 1, 2)
        dst_pts = np.float32([keypoints_ref[m.trainIdx].pt for m in matches]).reshape(-1, 1, 2)
    ...
    def match_card(
        ...
        if matches and len(matches) > 0:
            ...
            good_matches, conf_scores = self._lowe_ratio_test(matches, des_test, des_ref)
        ...
        if len(good_matches) < self.min_matches:
            ...
            if matches_cc and len(matches_cc) >= self.min_matches:
                ...
                good_matches = matches_cc[: max(self.min_matches, int(len(matches_cc) * 0.8))]
        ...
        while len(remaining_matches) >= self.min_matches:
            H, num_inliers, mask_flat = self._estimate_homography(kp_test, kp_ref, remaining_matches)
            ...
            inlier_matches = [m for m, flag in zip(remaining_matches, mask_bool) if flag]
            inlier_conf = remaining_conf[mask_bool] if remaining_conf.size else np.array([])
            ...
            conf_ransac = float(len(inlier_matches)) / float(len(remaining_matches)) if len(remaining_matches) > 0 else 0.0
            ...
            iter_count += 1
            if (not self.allow_multiple_instances) or (iter_count >= self.max_detections_per_ref):
                break

            # Remove inlier matches from remaining and continue searching
            remaining_matches = [m for m, flag in zip(remaining_matches, ~mask_bool) if flag]
    ...
    def visualize(
        ...
        ax.imshow(img_rgb)
        ax.set_title(f"UNO Card Detection: {len(detections)} cards found", fontsize=14, fontweight="bold")
